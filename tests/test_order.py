...
class TestGetListOrders:
    ...
    def test_get_list_orders_default_params_success_200(self):
        status_code, response_data = OrderMethods().get_list_orders()
        assert status_code == 200 and 'orders' in response_data
    ...
    def test_get_list_orders_id_200(self, authorization_courier):
        ...
        courier_id = response[1].get('id')
        if courier_id:
            payload = {"courierId": str(courier_id)}
            status_code, list_orders = OrderMethods().get_list_orders_sort(payload)
            assert status_code == 200 and list_orders != []
        else:
            pytest.fail("Не удалось получить id курьера для теста")
